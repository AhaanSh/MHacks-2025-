...
def get_rental_listings(city: str, state: str, limit: int = 500, **filters) -> pd.DataFrame:
    """
    Get rental listings from RentCast API.
    """
    ...
    print(f"Fetching rental listings from {city}, {state}...")
    try:
...
def main():
    print("RentCast Rental Listings Data Extraction")
    print("=" * 50)
    ...
    try:
        raw_df = get_rental_listings(CITY, STATE, LIMIT, **filters)
        if raw_df.empty:
